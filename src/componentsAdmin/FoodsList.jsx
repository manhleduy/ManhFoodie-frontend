...
import { getAllFoods } from '../lib/adminUntils'
import FoodControlCard from './FoodControlCard'
const FoodsList = () => {
    ...
    useEffect(()=>{
        const fetchData= async()=>{
            await getAllFoods(setError, setLoading,setResponse);
        }
        fetchData();
    },[setFoodList])
    useEffect(()=>{
    ...
    if(!response || response==='undefined' || !foodList){
        return <div>there is nothing in here</div>
    }
    return (
        <div className='flex flex-col gap-10'>
            <h1 className='text-solidOne  w-full text-center' >All food</h1>
            <div>
                <p className='font-bold text-orange-400'>food name</p>
                <input 
            ...
            <div className='flex gap-3 flex-wrap justify-center items-center'>
                ...
                if(foodName===""){
                  return true;
                    ...
                    return item.name.includes(foodName);
                }}).filter((entry, index)=>index<=currentPage*20-1 &&index>=(currentPage-1)*20 )
                .map((item, index)=>(
                    <FoodControlCard food={item} index={index} setOpenFoodUpdate={setOpenFoodUpdate}/>
                ))}
            </div>
