...
import { Delete, Ticket } from 'lucide-react';
import { useAppContext } from '../context/AppContext';
import { deleteDiscount } from '../lib/adminUntils';
import DiscountUpdateForm from './DiscountUpdateForm';
// Custom component to handle the complex dashed separator
const Separator = () => (
  <div className="absolute top-0 left-[50px] w-4 h-full flex items-center justify-center flex-col z-10">
    <span className="relative flex h-full border-l-2 border-dashed border-[#e8e8e8]
      before:content-[''] before:absolute before:bottom-[-8px] before:left-1/2 before:-translate-x-1/2 before:w-4 before:h-4 before:rounded-xl before:bg-[#e8e8e8]
      after:content-[''] after:absolute after:top-[-8px] after:left-1/2 after:-translate-x-1/2 after:w-4 after:h-4 after:rounded-full after:bg-[#e8e8e8]"
    />
  </div>
);

const DiscountControlTicket = ({data}) => {
  const {setError, setLoading}= useAppContext();
  const [openUpdate, setOpenUpdate]= useState(false);
    return (
    
      <div className="persective-1000 ">
        
        <div className="relative h-[230px] w-[450px] flex text-[#2d2d2d] bg-white rounded-xl transition-all duration-300 ease-in-out z-10 gap-4 overflow-hidden
          custom-animation-card hover:custom-hover-animation-paused">
          
          <svg xmlns="http://www.w3.org/2000/svg" width={58} height={230} viewBox="0 0 64 150" fill="black" className='flex-shrink-0 bg-orange-400'>
          ...
          <div className="relative flex justify-between w-full custom-content-ticket">
            {/* The ::after and ::before pseudo-elements for light/shadow require custom-content-ticket class */}
            <div className="flex flex-col justify-between w-full p-[0.625rem] pr-3 gap-1">
                <div className="flex justify-between">
                    <div className="flex flex-col w-2/5">
                    <p className="text-[10px] leading-none text-[#aeaeae]">Name</p>
                    <p className="font-bold text-[#2d2d2d]">{data.name}</p>
                    </div>
                    <div className="flex flex-col w-2/5">
                    <p className="text-[10px] leading-none text-[#aeaeae]">Remaining</p>
                    <p className="font-bold text-[#2d2d2d]">{data.amount}</p>
                    </div>
                </div>
                <div>
                    <div className="flex flex-col">
                    <p className="text-[10px] leading-none text-[#aeaeae]">Detail:</p>
                    <p className="font-bold text-[#2d2d2d]">{data.detail}</p>
                    </div>
                </div>
                <div>
                    <div className="flex flex-col">
                    <p className="text-[10px] leading-none text-[#aeaeae]">Apply for deal with:</p>
                    <p className="font-bold text-[#2d2d2d]">{data.requirement}</p>

                    </div>
                </div>
                <div className='flex justify-between items-end'>
                    <div className="flex flex-col w-30">
                    <p className="text-[10px] leading-none text-[#aeaeae]">expiration date:</p>
                    <p className="font-bold text-[#2d2d2d]">{data.expiration_date.slice(0,10)}</p>
                    </div>
                    <button
                    onClick={(e)=>{
                        const deletE=async ()=>{
                            await deleteDiscount(setError, setLoading);
                        }
                        deletE()
                    }}
                     className='flex gap-1 cursor-pointer bg-orange-200 items-center p-2 rounded-2xl'>
...
export default DiscountControlTicket